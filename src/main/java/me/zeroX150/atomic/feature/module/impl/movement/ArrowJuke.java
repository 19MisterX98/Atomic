                        ...
                        for(int x = -2;x<3;x++) {
                            ...
                            for(int z = -2;z<3;z++) {
                                if (!Atomic.client.player.getBoundingBox().expand(expand.getValue()).offset(x,0,z).contains(arrowPos)) {
                                    ...
                                    if(Atomic.client.world.getBlockState(np).getMaterial().isReplaceable() && Atomic.client.world.getBlockState(np.add(0,1,0)).getMaterial().isReplaceable()) {goTo = cp;break;}
                                }
                            }
                        }
                        if (goTo != null) {
